...
class DlAupController extends JController {
    ...
    function syncOtherDownload() {
        $this->documents = array();
        $this->prepareDocumentData();
//            var_dump($this->documents);
        $this->syncDocumentOtherDownloadsTable();
    }
    ...
    private function syncDocumentOtherDownloadsTable() {
//		$this->prepareDownloadDateTable();
        $query = 'SELECT document_id, user_id, downloaded_date FROM #__document_download_date';
        $db = JFactory::getDbo();
        $db->setQuery($query);
        ...
        foreach ($res as $obj) {
            ...
            // if (in_array((int)$obj->document_id, $this->documents) || in_array($obj->document_id, $this->documents)) {
            if (!empty($this->documents[$obj->document_id])) {
                $uploader_id = $this->documents[$obj->document_id]->uploader_id;
                if ($obj->user_id != $uploader_id) {
                    $uploader = JFactory::getUser($uploader_id);
                    // ok, good, insert aup data
                    // var_dump($this->documents[$obj->document_id]->number);
                    $ref_message = '(sync)' . $uploader->name . ' downloaded your document ' . $this->documents[$obj->document_id]->number . ' (' . $obj->downloaded_date . ')';
                    $this->updateScore(USER_SCORE_OTHER_USER_DOWNLOAD, $uploader_id, $ref_message);
                    $count++;
                } else {
                    $selfDownload++;
                }
